...
class GreedyProblemRepository(TSPProblemRepository):
    def run(self, selected_pois, n_days):
        ...
        while is_any_poi_able_to_assign: # if any POI able to be assigned
            ...
            while not poi_has_assigned:
                i = 0
                sorted_pois = sorted(selected_pois, key=lambda poi_id: self.get_poi_rating(poi_id), reverse=True) #sort by rating
                while i < len(sorted_pois) and not poi_has_assigned:
                    selected_poi = sorted_pois[i]
